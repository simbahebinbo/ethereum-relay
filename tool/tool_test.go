...
func Test_UnlockETHWallet(t *testing.T) {
	...
	// 下面是签名的测试
	tx := types.NewTx(&types.LegacyTx{ // 创建一个测试用的交易数据结构体
		Nonce:    123,                       // nonce 交易序列号
		To:       &common.Address{},         // to 接收者地址
		Value:    new(big.Int).SetInt64(10), // value 数值
		Gas:      1000,                      // gasLimit
		GasPrice: new(big.Int).SetInt64(20), // gasPrice
		Data:     []byte("交易"),              // data
	})
